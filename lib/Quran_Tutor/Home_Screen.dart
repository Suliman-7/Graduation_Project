...
class DataSearch extends SearchDelegate {
  ...
  Widget buildSuggestions(BuildContext context) {
    return FutureBuilder(
        ...
        builder:
            (BuildContext context, AsyncSnapshot<List<SearchVerse>> snapshot) {
          if (snapshot.hasData) {
            List<SearchVerse>? SearchV = snapshot.data;
            List SV = [];
            for (int i = 0; i < SearchV!.length; i++) {
              SV.add(SearchV[i].searchverse.toString());
              SV[i].toString().replaceAll("بسم الله الرحمن الرحيم ", "");
            }
