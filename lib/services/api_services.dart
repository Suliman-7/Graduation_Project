...
class ApiService {
  ...
  Future<List<SearchVerse>> getSearched() async {
    int index = 0;
    final endPointUrl =
        "https://api.alquran.cloud/v1/search/${" "}/all/quran-simple-clean";
  ...
  Future<List<Reciter>> getReciterList() async {
    ...
    jsonDecode(res.body).forEach((Element) {
      if (ReciterList.length < 10) ReciterList.add(Reciter.fromJSON(Element));
    });
  ...
  Future<surahList> getverses(int index) async {
    final Url =
        "https://api.alquran.cloud/v1/surah/${index}/editions/quran-simple";
  ...
  Future<verseCList> getVC(int index) async {
    final Url =
        "https://api.alquran.cloud/v1/surah/${index}/editions/quran-simple-clean";
    var res = await http.get(Uri.parse(Url));
  ...
  Future<verseDay> getVerseDay() async {
    String url =
        "http://api.alquran.cloud/v1/ayah/${random(1, 6237)}/editions/quran-simple,en.pickthall";
